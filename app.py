...
# ... (остальные функции-вычислители run_cma_es, run_symbolic_regression и т.д. остаются без изменений) ...
@st.cache_data
def run_cma_es(func_name, ngen_cма, sigma, num_individuals=10):
...
# --- РЕНДЕРЕР ДЛЯ ЧАСТИ 1 (С НОВЫМ ФУНКЦИОНАЛОМ) ---
...
def render_chapter_1():
    ...
    with col1:
        ...
        # 1. Словарь со сценариями
        SCENARIOS = {
            "Ручной режим": {
                "disabled": False,
                "description": "Настройте все параметры вручную для собственного исследования."
            },
            "Здоровая эволюция": {
                "disabled": True, "cxpb": 0.9, "mutpb": 0.05,
                "description": "Сбалансированный режим, демонстрирующий эффективную сходимость."
            },
            "Застой (без мутаций)": {
                "disabled": True, "cxpb": 0.9, "mutpb": 0.0,
                "description": "Сценарий показывает эволюционный тупик, когда без новых 'идей' (мутаций) алгоритм быстро застревает в локальном оптимуме."
            },
            "Хаос (гипермутация)": {
                "disabled": True, "cxpb": 0.9, "mutpb": 0.6,
                "description": "Высокая мутация разрушает найденные удачные решения быстрее, чем скрещивание успевает их закрепить. Обучение не происходит."
            },
            "Поиск вслепую (без скрещивания)": {
                "disabled": True, "cxpb": 0.0, "mutpb": 0.05,
                "description": "Без скрещивания алгоритм теряет свою главную силу — способность комбинировать решения. Эволюция превращается в медленный параллельный случайный поиск."
            }
        }

        # 2. Радио-кнопка для выбора сценария
        selected_scenario_name = st.radio(
            "Выберите сценарий-эксперимент:",
            options=SCENARIOS.keys()
        )
        
        params = SCENARIOS[selected_scenario_name]
        st.info(params["description"])

        is_disabled = params["disabled"]

        # 3. Ползунки, которые управляются сценариями
        # Значения по умолчанию берутся из словаря, если ключ существует
        ...
        # Эти параметры остаются управляемыми вручную во всех режимах
    ...
    **Интерпретация таблицы:**
    - **Столбцы "Мин.", "Макс.", "Сред.":** Цветовая шкала (от фиолетового к желтому) показывает рост приспособленности.
    - **Столбец "Ст. откл.":** Цветовая шкала (от желтого к фиолетовому) показывает падение генетического разнообразия.
    """)

# ... (все остальные функции render_chapter_2, 3, 4, 5 остаются без изменений) ...
...
def render_chapter_5():
    ...
    with col2:
        ...
        st.markdown("Для определения 'похожести' пользователей используется **коэффициент корреляции Пирсона**.")
        st.latex(r'''
        r = \frac{\sum_{i=1}^{n}(x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\sum_{i=1}^{n}(x_i - \bar{x})^2}\sqrt{\sum_{i=1}^{n}(y_i - \bar{y})^2}}
        ''')
        st.markdown(r"""
        - $r$: Коэффициент корреляции (от -1 до 1).
        - $n$: Количество фильмов, оцененных обоими.
        - $x_i, y_i$: Оценки $i$-го фильма.
        - $\bar{x}, \bar{y}$: Средние оценки пользователей.
        """)
